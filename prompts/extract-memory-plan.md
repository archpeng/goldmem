...
- `reminderCandidateIndex`: required when action is `create_reminder_candidate`; optional for `update_existing_reminder_candidate` when the update is already represented by a context link to the target reminder
...
- Use `update_existing_reminder_candidate` when the transcript changes or fills details for an open reminder; include `targetReminderId` and add a context link to that reminder.
